...
def main(look_back=1, duration = 180):
    ...
    ind              = 0
    look_back        = 1
    current_date     = str((datetime.now() - timedelta(1)).date().strftime("%m-%d-%Y"))
    AAVE_API_RESULT  = requests.get('https://aave-api-v2.aave.com/data/liquidity/v2?poolId=0xb53c1a33016b2dc2ff3653530bff1848a515c8c5&date='+current_date).json() #12-14-2021'
    AAVE_DATA        = pd.DataFrame(columns=['Token', 'CurrentPrice', 'availableLiquidity','decimals', 'totalLiquidity','utilizationRate','Assets'])
    for token in AAVE_API_RESULT:
        try:
            AAVE_DATA.loc[ind] = [token['symbol']] + [float(token['referenceItem']['priceInUsd'])] + \
    ...
    AAVE_DATA['totalLiquidity']       = AAVE_DATA['totalLiquidity'] * AAVE_DATA['CurrentPrice']
    AAVE_DATA['Assets']               = AAVE_DATA['Assets'] * AAVE_DATA['CurrentPrice']
    AAVE_DATA['Position']             = AAVE_DATA['Assets'] - AAVE_DATA['totalLiquidity']
    AAVE_DATA['Position($,Billion)']  = AAVE_DATA['Position'] / pow(10, 9)
    ...
    TOTAL_LIABILITIES = AAVE_DATA['totalLiquidity'].sum()
    TOTAL_ASSETS      = AAVE_DATA['Assets'].sum()
